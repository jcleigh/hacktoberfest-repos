                    ...
                    <div key={`repo-${i}`}>
                        <ul>
                            <li key={`li-${item.full_name}`}>
                                <a href={item.html_url} target="_blank">{item.full_name}</a> | Stars: {parseInt(item.stargazers_count).toLocaleString('en-US')} | Watchers: {parseInt(item.watchers_count).toLocaleString('en-US')}
